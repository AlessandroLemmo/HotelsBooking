...
public class HotelController {
    ...
    public String listRooms(Model model) {
        ...
        rooms = roomService.findAllByCriteria(searchRoom.getHotel().getCity(), searchRoom.getNumber(), searchRoom.getType(), searchRoom.getDirection());
    ...
    public String searchRooms(Room searchRoom, Model model, HttpServletRequest httpServletRequest,
        ...
        this.hotels = hotelService.findAllByCriteria(searchRoom.getHotel().getName(), searchRoom.getHotel().getCity(), null);
        return "redirect:/";
    }
    ...
    public String createRoom(@Valid Room room, BindingResult bindingResult, Model model,
        ...
        this.hotels = hotelService.findAllByCriteria(room.getHotel().getName(), null, null);
        if(hotels.size() == 1)
        	this.hotel = hotels.get(0);
        else {
        	model.addAttribute("message", new Message("error",
                    messageSource.getMessage("hotel_name_save_error_text", new Object[]{}, locale)));
        	setMainAttributes(model);
            setRoomAttributes(model, room);
        	return "new";
        }
